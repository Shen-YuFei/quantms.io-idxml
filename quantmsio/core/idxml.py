...
class IdXML:
    ...
    def _parse_protein_identifications(self) -> None:
        ...
        try:
            ...
            for protein_id in root.findall('.//ProteinIdentification'):
                for protein_hit in protein_id.findall('.//ProteinHit'):
                    ...
                    if accession:
                        ...
                        is_decoy = 0
                        target_decoy_param = protein_hit.find('.//UserParam[@name="target_decoy"]')
            ...
            logging.error(f"Unexpected error while parsing protein identifications: {e}")
            raise
        ...
        try:
            ...
            for peptide_id in root.findall('.//PeptideIdentification'):
                ...
                for peptide_hit in peptide_id.findall('.//PeptideHit'):
                    peptide_data = self._parse_peptide_hit(peptide_hit, mz, rt, scan_number)
                    if peptide_data:
            ...
            logging.error(f"Unexpected error while parsing peptide identifications: {e}")
            raise
    ...
    def _parse_peptide_hit(self, peptide_hit: ET.Element, mz: float, rt: float, scan_number: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            return {
                ...
                'rt': rt,
                'reference_file_name': self._extract_filename_from_spectrum_ref(scan_number),
    ...
    def _generate_peptidoform_msstats(self, sequence: str, modifications: List[Dict]) -> str:
        """
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        required_columns = [
            ...
            'additional_scores', 'mp_accessions', 'rt', 'reference_file_name',
            'spectrum_ref', 'q_value', 'consensus_support'
        ]
